...
var map$5 = new type('tag:yaml.org,2002:map', {
  kind: 'mapping',
  construct: function (data) { return data !== null ? data : {}; }
});
...
var Templates = /** @class */ (function () {
    ...
    Templates.prototype.replace = function (input, moment) {
 ...
 * @returns {Context}
 */
function configure$4(base, extension) {
  let index = -1;
  /** @type {string} */
  let key;
  ...
  if (extension.extensions) {
    while (++index < extension.extensions.length) {
      configure$4(base, extension.extensions[index]);
    }
  }
 ...
 * @returns {string}
 */
function containerFlow$2(parent, context) {
 ...
 */
function blockquote$2(node, _, context) {
  const exit = context.enter('blockquote');
  const value = indentLines$2(containerFlow$2(node, context), map$4);
  exit();
  return value
}
...
function patternInScope$2(stack, pattern) {
  return (
    listInScope$2(stack, pattern.inConstruct, true) &&
    !listInScope$2(stack, pattern.notInConstruct, false)
  )
}
 ...
 * @returns {boolean}
 */
function listInScope$2(stack, list, none) {
  if (!list) {
    return none
  }
 ...
 * @param {Break} _
 */
function hardBreak$2(_, _1, context, safe) {
  let index = -1;
  ...
  while (++index < context.unsafe.length) {
    ...
    if (
      ...
      patternInScope$2(context.stack, context.unsafe[index])
    ) {
 ...
 */
function longestStreak$1(value, character) {
 ...
 * @returns {string}
 */
function safe$2(context, input, config) {
  ...
  while (++index < context.unsafe.length) {
    ...
    if (!patternInScope$2(context.stack, pattern)) {
      continue
    }

    const expression = patternCompile$3(pattern);
    /** @type {RegExpExecArray|null} */
    let match;
  ...
  while (++index < positions.length) {
    ...
    if (start !== position) {
      ...
      // do the trick
      result.push(escapeBackslashes$2(value.slice(start, position), '\\'));
    }
  ...
  result.push(escapeBackslashes$2(value.slice(start, end), config.after));
 ...
 * @returns {string}
 */
function escapeBackslashes$2(value, after) {
  const expression = /\\(?=[!-/:-@[-`{-~])/g;
  /** @type {Array.<number>} */
  const positions = [];
  /** @type {Array.<string>} */
...
function code$3(node, _, context) {
  const marker = checkFence$1(context);
  ...
  if (formatCodeAsIndented$1(node, context)) {
    ...
    if (node.lang) {
      ...
      value += safe$2(context, node.lang, {
        before: '`',
        after: ' ',
        encode: ['`']
      });
...
const characterEscape$2 = /\\([!-/:-@[-`{-~])/g;
const characterReference$2 = /&(#(\d{1,7}|x[\da-f]{1,6})|[\da-z]{1,31});/gi;
...
function association$1(node) {
  ...
  return node.identifier
    .replace(characterEscape$2, '$1')
    .replace(characterReference$2, decodeIfPossible$1)
}
...
function definition$3(node, _, context) {
  const marker = checkQuote$1(context);
  const suffix = marker === '"' ? 'Quote' : 'Apostrophe';
  const exit = context.enter('definition');
  let subexit = context.enter('label');
  let value =
    '[' + safe$2(context, association$1(node), {before: '[', after: ']'}) + ']: ';
  ...
  if (node.title) {
    ...
    value +=
      ' ' +
      marker +
      safe$2(context, node.title, {before: marker, after: marker}) +
      marker;
    subexit();
  }
 ...
 * @returns {string}
 */
function containerPhrasing$3(parent, context, safeOptions) {
  const indexStack = context.indexStack;
  const children = parent.children || [];
  /** @type {Array.<string>} */
  const results = [];
...
emphasis$2.peek = emphasisPeek$1;
 ...
 * @param {Emphasis} node
 */
function emphasis$2(node, _, context) {
  const marker = checkEmphasis$1(context);
  const exit = context.enter('emphasis');
  const value = containerPhrasing$3(node, context, {
    before: marker,
    after: marker
  });
  exit();
 ...
 * @param {Emphasis} _
 */
function emphasisPeek$1(_, _1, context) {
 ...
 * @returns {string}
 */
function toString$2(node, options) {
...
function heading$2(node, _, context) {
  ...
  const subexit = context.enter('phrasing');
  let value = containerPhrasing$3(node, context, {before: '# ', after: '\n'});
...
function image$3(node, _, context) {
  const quote = checkQuote$1(context);
  const suffix = quote === '"' ? 'Quote' : 'Apostrophe';
  const exit = context.enter('image');
  let subexit = context.enter('label');
  let value = '![' + safe$2(context, node.alt, {before: '[', after: ']'}) + '](';
...
imageReference$2.peek = imageReferencePeek$1;

/**
 * @type {Handle}
 * @param {ImageReference} node
 */
function imageReference$2(node, _, context) {
  const type = node.referenceType;
  const exit = context.enter('imageReference');
  let subexit = context.enter('label');
  const alt = safe$2(context, node.alt, {before: '[', after: ']'});
  let value = '![' + alt + ']';
  ...
  const reference = safe$2(context, association$1(node), {before: '[', after: ']'});
  subexit();
  context.stack = stack;
  exit();
...
inlineCode$3.peek = inlineCodePeek$2;

/**
 * @type {Handle}
 * @param {InlineCode} node
 */
function inlineCode$3(node, _, context) {
...
function link$2(node, _, context) {
  ...
  if (formatLinkAsAutolink$1(node, context)) {
    // Hide the fact that we’re in phrasing, because escapes don’t work.
    const stack = context.stack;
    context.stack = [];
    exit = context.enter('autolink');
    value =
      '<' + containerPhrasing$3(node, context, {before: '<', after: '>'}) + '>';
    exit();
    context.stack = stack;
    return value
  }
...
function linkPeek$1(node, _, context) {
  return formatLinkAsAutolink$1(node, context) ? '<' : '['
}
...
linkReference$2.peek = linkReferencePeek$1;

/**
 * @type {Handle}
 * @param {LinkReference} node
 */
function linkReference$2(node, _, context) {
  const type = node.referenceType;
  const exit = context.enter('linkReference');
  let subexit = context.enter('label');
  const text = containerPhrasing$3(node, context, {before: '[', after: ']'});
  let value = '[' + text + ']';
...
function checkBulletOther(context) {
  const bullet = checkBullet$2(context);
 ...
 * @param {List} node
 */
function list$3(node, parent, context) {
  const exit = context.enter('list');
  const bulletCurrent = context.bulletCurrent;
  /** @type {string} */
  let bullet = node.ordered ? checkBulletOrdered(context) : checkBullet$2(context);
  /** @type {string} */
  ...
  if (!node.ordered) {
    ...
    //
    // …because otherwise it would become one big thematic break.
    if (checkRule$2(context) === bullet && firstListItem) {
      let index = -1;
  ...
  const value = containerFlow$2(node, context);
  context.bulletLastUsed = bullet;
  context.bulletCurrent = bulletCurrent;
  exit();

  // adjust string a little bit
  //@ts-ignore
...
function listItem$3(node, parent, context) {
  debugger;
  const listItemIndent = checkListItemIndent$2(context);
  let bullet = context.bulletCurrent || checkBullet$2(context);
  ...
  const value = indentLines$2(containerFlow$2(node, context), map);
  exit();
 ...
 */
function paragraph$2(node, _, context) {
...
function root$2(node, _, context) {
  return containerFlow$2(node, context)
}
 ...
 * @param {Strong} node
 */
function strong$2(node, _, context) {
  const marker = checkStrong$1(context);
 ...
 * @param {Strong} _
 */
function strongPeek$1(_, _1, context) {
 ...
 * @param {Text} node
 */
function text$5(node, _, context, safeOptions) {
  return safe$2(context, node.value, safeOptions)
}
 ...
 */
function thematicBreak$3(_, _1, context) {
  const value = (
    checkRule$2(context) + (context.options.ruleSpaces ? ' ' : '')
...
function joinDefaults$1(left, right, parent, context) {
  ...
  if (
    right.type === 'code' &&
    formatCodeAsIndented$1(right, context) &&
    (left.type === 'list' ||
      (left.type === right.type && formatCodeAsIndented$1(left, context)))
  ) {
    return false
  }
  ...
  if ('spread' in parent && typeof parent.spread === 'boolean') {
    if (
      ...
      (left.type === right.type ||
        ...
        (right.type === 'heading' && formatHeadingAsSetext$1(right, context)))
    ) {
      return
    }
...
function toMarkdown$9(tree, options = {}) {
  ...
  configure$4(context, {unsafe: unsafe$5, join: join$1, handlers: handle$1});
  configure$4(context, options);

  if (context.options.tightDefinitions) {
    configure$4(context, {join: [joinDefinition$1]});
  }

  /** @type {Handle} */
  context.handle = zwitch$1('type', {
    invalid: invalid$1,
    // @ts-expect-error: hush.
    unknown: unknown$1,
    // @ts-expect-error: hush.
...
function remarkStringify$1(options) {
...
function frontmatter(options) {
  ...
  add('toMarkdownExtensions', toMarkdown$8(options));
...
function metadataWriter(options, input) {
    ...
    var transformer = function (ast, vFile, next) {
        ...
        if (options.updateHeader && hasMetadata) {
            ...
            metadataNode.value = mergeValues(metadataNode.value, newValues(options.newHeaderTemplate, input));
        }
...
function mergeValues(currentValues, newValues) {
    ...
    var tagUnion = Array.from(new Set(__spreadArray(__spreadArray([], currentTags), newTags)).values());
...
function toMarkdown$7(opts = {}) {
  ...
  function handler(node, _, context) {
    ...
    const nodeValue = safe$2(context, node.value, { before: '[', after: ']' });
    const nodeAlias = safe$2(context, node.data.alias, { before: '[', after: ']' });
...
function wikiLinkPlugin(opts = {}) {
  ...
  add("toMarkdownExtensions", toMarkdown$7(opts));
}
...
var containerPhrasing$2 = phrasing$2;
...
function handleDelete(node, _, context) {
  ...
  var value = containerPhrasing$2(node, context, {before: '~', after: '~'});
  exit();
  return '~~' + value + '~~'
}
...
var patternCompile_1$2 = patternCompile$2;
...
inlineCode$2.peek = inlineCodePeek$1;



function inlineCode$2(node, parent, context) {
  ...
  while (++index < context.unsafe.length) {
    ...
    expression = patternCompile_1$2(pattern);
...
function listItem$2(node, parent, context) {
...
function listItemWithTaskListItem(node, parent, context) {
  var value = listItem_1$1(node, parent, context);
  var head = node.children[0];
...
function toMarkdown$2(options) {
  var config = configure_1$1(
    ...
    {
      extensions: [toMarkdown$6, toMarkdown$5, toMarkdown_1$1(options), toMarkdown$3]
    }
  );
...
function math() {
  ...
  add('toMarkdownExtensions', toMarkdown$1);
...
var text_1$1 = createCommonjsModule(function (module, exports) {
...
var contentInitial = {
  91: definition_1$1 // Left square bracket
};
var flowInitial = {
  '-2': codeIndented_1,
  // Horizontal tab
...
function prettifier(content, options, frontMatterData) {
    ...
    if (frontMatterData === void 0) { frontMatterData = { today: moment(), tags: [] }; }
    var processor = remark();
    if (options.createHeaderIfNotPresent || options.updateHeader) {
        processor = processor
            .use(remarkFrontmatter)
            .use(metadataWriter, options, frontMatterData);
    }
    ...
    if (!stringifyOptions.newlinesAroundHeadings) {
        stringifyOptions.join = [
            //@ts-ignore
            function (left, right) {
                if (left.type === 'yaml' || right.type === 'yaml') {
                    return 1;
                }
                return 0;
            },
    ...
    processor = processor
        ...
        .use(remarkMath)
        .use(remarkGfm)
        .use(remarkStringify$1, stringifyOptions);
...
var MarkdownPrettifier = /** @class */ (function (_super) {
    ...
    MarkdownPrettifier.prototype.runPrettifier = function () {
        ...
        if (view instanceof obsidian.MarkdownView) {
            ...
            // Remember the cursor
            var cursor_1 = editor_1.getCursor();
            editor_1.execCommand("selectAll");
            var text_1 = editor_1.getSelection();
            prettifier(text_1, this.settings, { today: moment(), tags: [] })
                ...
                try {
                    ...
                    var n_before = String(data).split(/\r\n|\r|\n/).length;
                    var n_after = String(text_1).split(/\r\n|\r|\n/).length;
                    var lines_changed = n_before - n_after;
                    if (lines_changed != 0) {
                        if (lines_changed > 0) {
                            new obsidian.Notice("Prettifier: added " + lines_changed + " lines.");
                        }
                        else {
                            new obsidian.Notice("Prettifier: removed " + lines_changed + " lines.");
                        }
                    }
                    // Update the cursor
                    if (cursor_1.line) {
                ...
                editor_1.replaceSelection(output, "start");
    ...
    MarkdownPrettifier.prototype.updateMatters = function () {
        ...
        if (view instanceof obsidian.MarkdownView) {
            // Do work here
            var editor_2 = view.sourceMode.cmEditor;
            // Remember the cursor
            var cursor_2 = editor_2.getCursor();
            editor_2.execCommand("selectAll");
            var text = editor_2.getSelection();
            var frontMatterData = {
                today: moment(),
            ...
            prettifier(text, this.settings, frontMatterData)
                .then(function (data) {
                editor_2.replaceSelection(String(data), "start");
...
var MarkdownPrettifierSettingsTab = /** @class */ (function (_super) {
    ...
    function MarkdownPrettifierSettingsTab(app, plugin) {
        ...
        _this.templateExample = document.createElement('b');
        _this.templateExampleValidity = document.createElement('b');
        _this.plugin = plugin;
        return _this;
    }
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            if (!new Templates().isValidYaml(this.plugin.settings.newHeaderTemplate)) return [3 /*break*/, 2];
                            return [4 /*yield*/, this.plugin.saveSettings()];
                        case 1:
                            _a.sent();
                            _a.label = 2;
    ...
    MarkdownPrettifierSettingsTab.prototype.updateTemplateExample = function () {
        ...
        var isValid = templateLib.isValidYaml(this.templateExample.innerText);
        this.templateExampleValidity.innerText = isValid ? "is valid YAML" : "is NOT valid YAML";
    };
    MarkdownPrettifierSettingsTab.prototype.newHeaderHelp = function () {
    ...
    MarkdownPrettifierSettingsTab.prototype.newHeaderTemplateHelp = function () {
        var descEl = document.createDocumentFragment();
    ...
    MarkdownPrettifierSettingsTab.prototype.dateFormattingDescription = function (descEl) {
